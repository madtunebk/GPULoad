...
#[derive(Parser)]
struct Args {
    #[arg(long, default_value = "temp/latents.safetensors")]
    latents: String,

    #[arg(long = "output", default_value = "temp/output.png")]
    out: String,
...
fn load_vae(vae_path: &std::path::Path, device: &Device) -> Result<AutoEncoderKL> {
    let vb = unsafe {
        VarBuilder::from_mmaped_safetensors(&[vae_path], DType::F32, device)
            .with_context(|| format!("loading VAE weights from {}", vae_path.display()))?
    };
...
pub fn run() -> Result<()> {
    ...
    let (devices, use_multi) = {
        if args.multi_gpu {
            ...
            (vec![Device::new_cuda(args.gpu)?], false)
        }
    };
    ...
    let lat_map: HashMap<String, Tensor> = candle_core::safetensors::load(&args.latents, &Device::Cpu)
        .with_context(|| format!("loading latents from {}", args.latents))?;
    let key = if lat_map.contains_key("latents") {
        "latents"
    } else {
        lat_map
            .keys()
            .find(|k| *k != "width" && *k != "height")
            .context("no latents tensor found")?
    };
    let packed = lat_map[key].to_dtype(DType::F32)?;
    ...
    let decoded = if args.tile_size == 0 {
        ...
        let lat = latents_cpu.to_device(&devices[0])?;
        tiled_decode(&vaes[0], &lat, height, width, args.tile_size, args.tile_overlap)?
    };
