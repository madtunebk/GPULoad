...
use sd_minimal::{device_config, lora, path_config};
...
fn encode_clip(prompt: &str, device: &Device, dtype: DType, lora_path: Option<&str>, lora_scale: f32) -> Result<Tensor> {
    ...
    // Try local models/tokenizer/ first, then fall back to HF snapshot (tokenizer/tokenizer.json).
    let clip_tokenizer_path = {
        let local = path_config::clip_tokenizer_path();
        if local.exists() {
            local
        } else {
            hf_path("tokenizer/tokenizer_config.json")?
        }
    };
...
fn run() -> Result<()> {
    ...
    std::fs::create_dir_all("temp").context("creating temp directory for prompt embeddings")?;
